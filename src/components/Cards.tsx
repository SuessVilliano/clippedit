      ...
      <div className="card-body">
        ...
        <div className="meter">
          <div className="meter-track">
            <div className="meter-fill" style={{ width: `${s.momentum ?? 0}%` }} />
          </div>
        ...
        <div className="stats">
          ...
          <div className="stat">
            ...
            <div className={`value ${(s.growth5mPct ?? 0) >= 0 ? "up" : "down"}`}>
              {s.growth5mPct == null ? "—" : `${s.growth5mPct >= 0 ? "▲" : "▼"} ${Math.abs(s.growth5mPct).toFixed(0)}%`}
            </div>
          </div>
        </div>
        ...
        <div className="card-actions">
          ...
          <ClipButton
            target={{ title: s.title || s.displayName, platform: s.platform, creator: s.displayName, url: s.url, kind: "live" }}
          />
        </div>
      </div>
    </article>
...
export function ClipCardView({ c, index = 0 }: { c: ClipCard; index?: number }) {
  return (
    <article className="card" style={{ animationDelay: `${Math.min(index * 40, 400)}ms` }}>
      ...
      />
      <div className="card-body">
        <div className="card-title" style={{ WebkitLineClamp: 2, opacity: 1, fontWeight: 600 }}>
          {c.title ?? "Untitled clip"}
        </div>
        <div className="card-sub">
          {c.creator ? `by ${c.creator}` : "—"}
          {c.ageHours != null ? ` · ${c.ageHours}h old` : ""}
        </div>

        <div className="stats">
          <div className="stat">
            <div className="label">Views/hr</div>
            <div className="value up">{formatNumber(c.viewsPerHour)}</div>
          </div>
          <div className="stat">
            <div className="label">Total</div>
            <div className="value">{formatNumber(c.views)}</div>
          </div>
        </div>
        ...
        <div className="card-actions">
          ...
          <ClipButton
            target={{ title: c.title ?? "Clip", platform: c.platform, creator: c.creator, url: c.url, kind: "clip" }}
            label="Produce"
          />
        </div>
      </div>
  ...
  return (
    <article className="card" style={{ animationDelay: `${Math.min(index * 40, 400)}ms` }}>
      <Thumb
        ...
        overlay={
          <>
            <span className="badge live" style={m.kind === "clip" ? { background: "rgba(56,189,248,0.9)" } : undefined}>
              {m.kind === "live" ? (
                <>
      ...
      <div className="card-body">
        ...
        <div className="meter">
          ...
          <span className="meter-num">{m.score}</span>
        </div>
        ...
        <div className="card-actions" style={{ marginTop: 10 }}>
          {m.url ? (
            <a className="btn" href={m.url} target="_blank" rel="noreferrer">
              {m.kind === "live" ? "Watch live" : "Open clip"}
            </a>
          ) : null}
          <ClipButton
            target={{ title: m.title, platform: m.platform, creator: m.subtitle, url: m.url, kind: m.kind }}
          />
        </div>
      </div>
    </article>
